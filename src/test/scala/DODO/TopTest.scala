  ...
  it should "执行DHRYSTONE基准测试" in {
    test(new TopWithMemory) { dut =>
      ...
      for (cycle <- 1 to maxCycles) {
        ...
        val fetch_instB = dut.io.fetch_instA.peek().litValue()
        ...
        println(f"map_instA=0x${map_instA}%08X,fetch_instB=0x${map_instB}%08X,dis_instA=0x${dis_instA}%08X,dis_instB=0x${dis_instB}%08X,dis_instC=0x${dis_instC}%08X,fetchblock=${fetchblock}%08X")
